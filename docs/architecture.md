...
- **Compatible publication.** One serialized CI deployment owns the Worker and Pages after all three required checks pass. It reads the live site's `/assistant-release.json` and loads that exact revision's reviewed corpus from Git, then checks and tests a Worker bundle containing the new and actual previous live corpus. The manifest also pins the assistant endpoint built into the browser; production rejects any endpoint other than `https://api.lyriclint.com/v1/answers`. Worker health must confirm both version/hash pairs before Pages publishes the exact E2E artifact; CI then verifies live site metadata. An already published revision is verified without redeploying or pruning compatibility. Each request's `clientRuleSetVersion` and `clientCorpusHash` select one known bundled corpus for prompts, cache keys, and citation validation. Hashless requests map only to an explicitly retained legacy corpus, never whichever corpus shares their version. Unknown pairs fail closed. This protects rollout and failed Pages publication without promising support for arbitrarily old tabs; [CI and deployment](ci.md) owns bootstrap, credentials, and retry procedure.
- **Model call.** OpenAI Responses API (`gpt-5.6-sol`, medium reasoning, `store: false`, 16,384 output tokens) through an authenticated Cloudflare AI Gateway, using LyricLint's OpenAI project API key. Strict structured output contains typed blocks, each carrying zero or more rule ids. The Worker rejects unknown ids, duplicate rich-reference assignments, more than four distinct references, reviewed claims without reviewed citations, and invented source ids — an invalid answer is a 502, never partially rendered. Every block and visible tool annotation (`show_lyrics`, `propose_edits`, and `manage_links` notes) stays in the language of the visitor's latest question across tool rounds and repairs; the selected song language, quoted lyrics, corpus, tool output, and earlier assistant prose never select the answer language. Prompt order is stable instructions → corpus → cache breakpoint → pruned history → question, cached under a key derived from the ruleset version and corpus hash.
