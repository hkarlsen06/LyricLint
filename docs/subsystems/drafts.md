...
second control for the thing the pointer is already on. The name draws nothing to advertise that (a bordered
field on every row is six boxes for a name that is usually only read); it just takes the
