...
- The developer token is not a secret: inlined as `PUBLIC_APPLE_MUSIC_TOKEN` (a **build**
  ...
  minting machine, capped at six months. `appleMusicConfigured` reads `exp`, because the real
...
- A remembered song is always pending, because asking MusicKit whether it holds a token means
...
- The search uses the developer token (no sign-in) against `music.storefrontId`. The link
  parser checks `?i=` before the path, because Apple's share sheet copies album URLs and the
  album id in the path is valid.
...
handed to every browser that loads the workbench, which is what a developer token _is_, so it is
...
- **There is a `playbackTimeDidChange` event**, so nothing here polls. Both other bridges run a
  ...
  **The seek hold is what keeps that gap off the screen, and its backstop had to learn the burst.**
  ...
  only an event that has moved somewhere that is neither the origin nor the target (a seek the
  player redirected or ignored) counts toward giving up, which is the one case the backstop is actually
...
queue around it before starting, which is MusicKit's only pre-start positioning.
...
picture plus two rows of chrome, and a width-only cap pushes the actions below the dialog's own foot
on a short window.
...
- **Both copies of the cover are decorative (`alt=""`).** The track is named in the row beside the
  ...
  (`View album art`), so an alt on either image would announce the same fact twice.
