...
- The audio drop `preventDefault()`s **only** when actually taking a file, because
  unconditionally preventing on `dragover` breaks CodeMirror's native text drop. Detection on `dragover`
...
the copied text, because the paste has no idea where in which document the copy was made. The
...
- **A copy with something to carry is owned outright; every other copy is left to CodeMirror
  untouched.** Adding a flavor beside the built-in handler's cannot work, since it opens with
...
- **Landed links go through `setSectionLinkEffect`, never the restore effect.** The restore is the
  draft being read back and is deliberately invisible to the shell's save, so links applied through it
...
taking the file, since unconditionally preventing on `dragover` is what breaks native text drop, and
the shell returning false hands the event straight back to CodeMirror.
