...
| `performer.inline-mismatch` | Suggestion | A styled slot the section legend does not yet name — one finding per slot, anchored on its first span; a section with no legend at all is `performer.header-required`'s finding | Choose the section and styled performers | `G-SECTIONS` |
