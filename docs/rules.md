...
| `performer.header-required` | Suggestion | A multi-vocalist section has inline differentiation but its header does not yet name the voices | Safely remove performer formatting, offered for sections that no longer need differentiation; the formatting-first stance is to keep it until the voices are known | `G-SECTIONS` |
...
| `performer.inline-mismatch` | Suggestion | A styled slot the section legend does not yet name, with one finding per slot, anchored on its first span; a section with no legend at all is `performer.header-required`'s finding | Choose the section and styled performers | `G-SECTIONS` |
...
| `quotes.typewriter` | Warning | Curly quotes, guillemets, low quotes, or a spacing acute accent beside a word occur in lyric text; in a Nordic language (`no`, `nb`, `nn`, `da`, `sv`, `is`, `fo`) any double quote that is not `«` or `»`, the straight `"` included | Safe replacement for quote marks; preview a straight apostrophe for an acute accent; in Nordic languages a straight `"` previews `«` or `»` by parity on the line; exclude unsupported markup | `G-TYPEWRITER` |
...
| `unknown.improvised-marker` | Suggestion | A bare run of question marks stands as its own token where words would go, as in `I heard ??? tonight`, or a single `?` occupies the whole line | Preview replacement, because the same characters can be deliberate punctuation | `G-UNKNOWN` |
...
contributor checklist for turning supplied guideline material into entries is
