...
export default defineConfig(({ mode }) => ({
	 ...
	 * as a DNS-rebinding defence, and answers 403 before any route runs, which
	...
	build: {
		 ...
		 *
		 * The alternative of minifying the files is off the table: their guidelines
	...
	plugins: [
		...
		sveltekit({
			 ...
			 * ignores, and SvelteKit drops it from the tag rather than emitting something
			...
			csp: {
				...
				directives: {
					...
					// Harper's `WorkerLinter` builds its worker from a `blob:` URL, since the
	...
	test: {
		 ...
		 * untracked file.
		 *
		 * The deployed build leaves `PUBLIC_SPOTIFY_CLIENT_ID` unset, since Spotify's
