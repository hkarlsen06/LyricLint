 ...
 */
async function draw(props: { text: string; mark?: ReturnType<typeof marker> }) {
	return (await render(CodeProse, props)).container;
}
...
describe('CodeProse', () => {
	...
	it('adds not one character to the sentence, and draws no marker', async () => {
		...
		const container = await draw({
			text: 'A time is written in digits: `8 a.m.`, never `8:00 A.M.`'
		});

		expect(container.textContent).toBe('A time is written in digits: 8 a.m., never 8:00 A.M.');
	});

	it('draws a sentence with no forms in it as plain text', async () => {
		const container = await draw({ text: 'Every ad-lib belongs in the transcription.' });

		expect(faces(container)).toEqual([]);
		expect(container.textContent).toBe('Every ad-lib belongs in the transcription.');
	});

	it('loses no words to an unpaired marker', async () => {
		const container = await draw({ text: 'a bare ` and the words after it' });

		expect(container.textContent).toBe('a bare  and the words after it');
	});

	it("draws a box, and never smaller than the ramp's floor", async () => {
