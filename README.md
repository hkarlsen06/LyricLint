...
card's shape moves. Generate everything with:

```sh
bun run render:all
```

This generates the social preview, all three product stills, both detail WebM/GIF loops,
and the hero desktop/mobile videos, including responsive images and the dimensions manifest.
It builds the app, starts a private HTTP preview server on an available loopback port,
and closes it on completion, failure, or Ctrl+C. Captures run sequentially against that
fixed build. Requires Chromium installed for Playwright, Node, ffmpeg, and ffprobe.
Generated files land in `static/`; run your normal build again before deploying them.

Individual captures are also available against a running dev/preview server
(default `http://127.0.0.1:5173`, overridden with `ORIGIN`):
...
bun run render:motion --hero         # the hero loop (desktop + mobile webm)
```
